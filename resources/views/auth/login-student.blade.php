            ...
            <div class="hidden lg:flex flex-col justify-center p-12 bg-primary-container text-white relative">
                <div class="relative z-10">
                    ...
                    <h2 class="text-4xl font-extrabold mb-6 leading-tight">
                        {{ __('messages.login_student_portal_headline') }}</h2>
                    <p class="text-lg text-white/90 mb-8 leading-relaxed">
                        {{ __('messages.login_student_portal_desc') }}
                    </p>
            ...
            <div class="p-8 sm:p-12 flex flex-col justify-center">
                <div class="mb-10">
                    <div
                        ...
                        <span>{{ __('messages.login_portal_student') }}</span>
                        <span>•</span>
                        <a href="{{ route('login') }}"
                            class="hover:underline text-on-surface-variant font-medium normal-case tracking-normal">{{ __('messages.login_change_portal') }}</a>
                    </div>
                    <h3 class="text-2xl font-bold text-on-surface mb-2">{{ __('messages.login_student_title') }}</h3>
                    <p class="text-on-surface-variant">{{ __('messages.login_student_desc') }}</p>
                </div>
                ...
                <form action="{{ route('login.student') }}" method="POST" class="space-y-6" novalidate>
                    ...
                    <div x-data="{ showPassword: false }">
                        <div class="flex justify-between mb-2">
                            ...
                            <a href="{{ route('password.request') }}"
                                class="text-sm font-bold text-primary-container hover:text-secondary transition-colors">{{ __('messages.login_forgot') }}</a>
                        </div>
                        <div class="relative">
                    ...
                    <div class="flex items-center">
                        <input id="remember-me" name="remember-me" type="checkbox"
                            class="h-4 w-4 text-primary-container focus:ring-primary-container border-outline-variant rounded">
                ...
                <div class="mt-8 text-center">
                    <p class="text-on-surface-variant text-sm">
                        ...
                        <a href="{{ route('register') }}"
                            class="font-bold text-primary-container hover:text-secondary transition-colors">{{ __('messages.login_register_student') }}</a>
                    </p>
                </div>
            </div>
        </div>
