...
<nav class="fixed top-0 left-0 right-0 z-[60] bg-white/80 backdrop-blur-xl border-b border-outline-variant/15" x-data="{ mobileOpen: false, userDropdownOpen: false }">
        ...
        <div class="flex justify-between h-20">
            ...
            <div class="flex items-center space-x-8 flex-shrink-0">
                <a href="/" class="flex items-center space-x-3 group">
                    <div class="w-10 h-10 bg-primary rounded-xl flex items-center justify-center flex-shrink-0 shadow-md shadow-primary/20 group-hover:scale-105 transition-transform">
                    ...
                    <div class="flex flex-col">
                        ...
                        <span class="text-[9px] font-black uppercase tracking-widest text-primary/80 mt-0.5">La Academia Digital</span>
                    </div>
                </a>
            ...
            <div class="hidden lg:flex items-center space-x-4">
                ...
                @auth
                    ...
                    @if (Auth::user()->role === 'tutor')
                        <div class="inline-flex items-center gap-2 px-4 py-2 bg-teal-500/10 text-teal-600 text-xs font-bold rounded-full border border-teal-500/20">
                            <span class="w-1.5 h-1.5 rounded-full bg-teal-500"></span>
                            Mentor
                        </div>
                    @else
                        <div class="inline-flex items-center gap-2 px-4 py-2 bg-primary/10 text-primary text-xs font-bold rounded-full border border-primary/20">
                ...
                <div class="relative" x-data="{ open: false }">
                    <button @click="open = !open" @click.away="open = false"
                        ...
                        <svg class="w-3 h-3 text-on-surface-variant transition-transform duration-200" :class="open ? 'rotate-180' : ''"
                ...
                @auth
                    ...
                    <div class="relative">
                        <button @click="userDropdownOpen = !userDropdownOpen" @click.away="userDropdownOpen = false" 
                            ...
                            <div class="w-8 h-8 rounded-full overflow-hidden bg-primary/10 flex items-center justify-center text-primary font-bold text-sm">
                            ...
                            <svg class="w-4 h-4 text-on-surface-variant pr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                            </svg>
                        </button>
                        ...
                        <div x-show="userDropdownOpen" 
                            ...
                            <div class="px-5 py-3 border-b border-outline-variant/15 flex items-center space-x-3">
                                <div class="w-10 h-10 rounded-full overflow-hidden bg-primary/10 flex items-center justify-center text-primary font-bold">
                                    @if($user->profile_photo_path)
                                        <img src="{{ $user->profile_photo_url }}" alt="{{ $user->name }}" class="w-full h-full object-cover">
                                    @else
                                        {{ strtoupper(substr($user->name, 0, 1)) }}
                                ...
                                <div class="overflow-hidden">
                                    ...
                                    <p class="text-[10px] text-on-surface-variant font-mono truncate">ID: {{ $user->student_id ?? $user->id }}</p>
                                </div>
                            </div>
                ...
                @else
                    <a href="{{ route('login') }}" class="text-sm font-bold text-on-surface-variant hover:text-primary transition-colors px-4 py-2">{{ __('messages.nav_access') }}</a>
                    <a href="{{ route('register') }}" class="btn-prestige text-sm px-6 py-2.5 shadow-md shadow-primary/10">{{ __('messages.welcome_btn_tutor') }}</a>
                @endauth
            </div>
            ...
            <div class="lg:hidden flex items-center space-x-3">
                ...
                <button @click="mobileOpen = !mobileOpen" 
                    ...
                    <svg class="w-6 h-6" :class="mobileOpen ? 'hidden' : 'block'" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16" />
                    </svg>
                    <svg class="w-6 h-6" :class="mobileOpen ? 'block' : 'hidden'" fill="none" stroke="currentColor" viewBox="0 0 24 24" style="display: none;">
    ...
    <div x-show="mobileOpen" 
        ...
        @auth
            ...
            <div class="flex items-center space-x-3 p-4 bg-surface-container/50 rounded-2xl border border-outline-variant/15 mb-4">
                <div class="w-12 h-12 rounded-xl overflow-hidden bg-primary/10 flex items-center justify-center text-primary font-extrabold text-lg">
                    @if($user->profile_photo_path)
                        <img src="{{ $user->profile_photo_url }}" alt="{{ $user->name }}" class="w-full h-full object-cover">
                    @else
                        {{ strtoupper(substr($user->name, 0, 1)) }}
            ...
            <div class="space-y-2">
                @if ($isTutorMode)
                    <div class="px-2 py-1 text-[10px] font-black uppercase tracking-widest text-primary/80 mb-2">Panel de Tutor</div>
                    <a href="{{ route('dashboard.tutor') }}" class="flex items-center space-x-3 px-4 py-3 rounded-xl font-bold text-on-surface hover:bg-surface-container {{ request()->routeIs('dashboard.tutor') && !request()->routeIs('dashboard.tutor.post') && !request()->routeIs('dashboard.tutor.history') ? 'bg-primary/5 text-primary' : '' }}">
                        <span>{{ __('messages.sidebar_resumen') }}</span>
                    </a>
                    <a href="{{ route('dashboard.tutor.post') }}" class="flex items-center space-x-3 px-4 py-3 rounded-xl font-bold text-on-surface hover:bg-surface-container {{ request()->routeIs('dashboard.tutor.post') ? 'bg-primary/5 text-primary' : '' }}">
                        <span>{{ __('messages.sidebar_offer') }}</span>
                    </a>
                    <a href="{{ route('dashboard.tutor.history') }}" class="flex items-center space-x-3 px-4 py-3 rounded-xl font-bold text-on-surface hover:bg-surface-container {{ request()->routeIs('dashboard.tutor.history') ? 'bg-primary/5 text-primary' : '' }}">
                        <span>{{ __('messages.sidebar_historial') }}</span>
                    </a>
                    <a href="{{ route('dashboard.tutor.materials') }}" class="flex items-center space-x-3 px-4 py-3 rounded-xl font-bold text-on-surface hover:bg-surface-container {{ request()->routeIs('dashboard.tutor.materials*') ? 'bg-primary/5 text-primary' : '' }}">
                        <span>Material</span>
                    </a>
                    <a href="{{ route('dashboard.tutor.profile') }}" class="flex items-center space-x-3 px-4 py-3 rounded-xl font-bold text-on-surface hover:bg-surface-container {{ request()->routeIs('dashboard.tutor.profile') ? 'bg-primary/5 text-primary' : '' }}">
                        <span>{{ __('messages.sidebar_perfil') }}</span>
                    </a>
                    

                @else
                    <div class="px-2 py-1 text-[10px] font-black uppercase tracking-widest text-secondary/80 mb-2">Panel de Estudiante</div>
                    <a href="{{ route('dashboard.student') }}" class="flex items-center space-x-3 px-4 py-3 rounded-xl font-bold text-on-surface hover:bg-surface-container {{ request()->routeIs('dashboard.student') ? 'bg-primary/5 text-primary' : '' }}">
                        <span>{{ __('messages.sidebar_resumen') }}</span>
                    </a>
                    <a href="{{ route('dashboard.search') }}" class="flex items-center space-x-3 px-4 py-3 rounded-xl font-bold text-on-surface hover:bg-surface-container {{ request()->routeIs('dashboard.search') ? 'bg-primary/5 text-primary' : '' }}">
                        <span>{{ __('messages.sidebar_buscar') }}</span>
                    </a>
                    <a href="{{ route('dashboard.history') }}" class="flex items-center space-x-3 px-4 py-3 rounded-xl font-bold text-on-surface hover:bg-surface-container {{ request()->routeIs('dashboard.history') ? 'bg-primary/5 text-primary' : '' }}">
                        <span>{{ __('messages.sidebar_historial') }}</span>
                    </a>
                    <a href="{{ route('dashboard.profile') }}" class="flex items-center space-x-3 px-4 py-3 rounded-xl font-bold text-on-surface hover:bg-surface-container {{ request()->routeIs('dashboard.profile') ? 'bg-primary/5 text-primary' : '' }}">
                        <span>{{ __('messages.sidebar_perfil') }}</span>
                    </a>
            ...
            <div class="pt-6 border-t border-outline-variant/15 mt-4">
                <form action="{{ route('logout') }}" method="POST">
                    @csrf
                    <button type="submit" class="w-full flex justify-center items-center gap-2 py-3 bg-red-50 text-red-600 font-bold rounded-xl text-sm transition-colors">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1"></path></svg>
        ...
        @else
            ...
            <div class="flex flex-col gap-3">
                <a href="{{ route('login') }}" class="w-full text-center py-3 font-bold text-on-surface hover:bg-surface-container rounded-xl transition-all">{{ __('messages.nav_access') }}</a>
                <a href="{{ route('register') }}" class="w-full text-center py-3 font-bold bg-primary text-white hover:bg-primary-dark rounded-xl shadow-md transition-all">{{ __('messages.welcome_btn_tutor') }}</a>
            </div>
        @endauth

        <!-- Language selections inside mobile drawer -->
        <div class="pt-6 border-t border-outline-variant/15 mt-4 space-y-2">
            <span class="text-xs font-bold text-on-surface-variant uppercase tracking-wider block mb-2">Idioma / Language</span>
